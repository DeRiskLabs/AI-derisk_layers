...
- [ ] `user_story_arg :current_authorization` (context-derived).
- [ ] `on_success` receives the named object (`articles:` / `article:`) and returns it
      directly — no payload hash, never a generic `result` key.
- [ ] `on_failure(<named_object>: nil, errors: nil)` accepts the named object or an
      errors collection and maps to `GraphQL::ExecutionError`s.
- [ ] Scoping implemented in the user story, driven by `current_authorization` — not in the
      resolver.
