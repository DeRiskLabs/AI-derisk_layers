...
# common_agent_skills/derisk_layers/authoring-graphql-queries/SKILL.md
...
6. `on_success(result:)` returns the result **directly** (not a payload hash).
...
| `on_success` | returns `{ <resource>:, errors: [] }`      | returns the result directly             |
...
- returning payload hashes from `on_success` — queries return the result itself.
