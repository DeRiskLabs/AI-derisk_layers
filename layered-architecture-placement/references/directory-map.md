...
Graph-facing user stories live in the MAIN app (`app/user_stories/graph/...`), not inside
...
Cross-cutting/domain logic for the main app goes under `app/lib/<type>/<domain>/`; user stories
go under `app/user_stories/`. Anything specific to an API or feature boundary lives in that
engine's mirror of this structure, with an engine-local base class over the gem's base.
