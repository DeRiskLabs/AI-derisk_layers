...
- [ ] A `context` for each branch the story can take:
  ...
  - [ ] `when not authorized` / `when it belongs to another user`
- [ ] Success asserts both the persisted effect (via `reload`) and the success callback.
...
- [ ] Effects asserted by reading real records (`reload`), not by inspecting internals.
